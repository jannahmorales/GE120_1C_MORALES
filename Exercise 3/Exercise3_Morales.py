...
while True:
    ...
    dep = getLatitude(azimuth=float(azimuth), distance=float(distance))
    ...
    # get corrected lat
    consCorrLat = -sumLat/sumDist #consCorrLat is -CL 
    consCorrDep = -sumDep/sumDist


# for line in lines:
    corr_lat = consCorrLat*float(distance) #get correction per line, line [1] is distance
    corr_dep = consCorrDep*float(distance)
...
print("\n\n")
print('{: ^10} {: ^10} {: ^15} {: ^15} {: ^10}'.format("Line No.", "Distance", "Bearing", "Latitude", "Departure"))
print("--------------------------------------------------------------------------------")
for line in lines:
